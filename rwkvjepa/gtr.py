"""Pure GTR (Global Temporal Retrieval + MLP) — the ICLR 2026 main-table model.

Ported verbatim from external/GTR/models/GTR.py into the TSL framework so it
can be run via Time-Series-Library/run.py. The only framework-level coupling is
the forward signature `forward(x_enc, cycle_index)`: the cycle_index is supplied
by the GTR cycle loader (data_loader.py, opt-in via --cycle>0) and dispatched in
exp_long_term_forecasting._model_call. With --cycle unset this model is never
selected for the standard 4-tuple path.

Config keys consumed (all present in run.py): seq_len, pred_len, enc_in, cycle,
d_model, dropout, use_revin (1/0), individual.
"""
import torch
...
class GTR(nn.Module):
    ...
    def forward(self, x, q):
        ...
        # Step 2: GTA mode, aggregate across channels (inter-variable dependencies).
        if self.agg:
...
class Model(nn.Module):
    def __init__(self, configs):
        ...
        self.use_revin = bool(configs.use_revin)
